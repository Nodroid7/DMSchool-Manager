...
export class EmailersCanteenNewsComponent implements OnInit {
  ...
  submit() {
    this.serverService.post('manager/' + this.request_type + '_news', {
      id: this.id,
  ...
  ngAfterViewInit(): void {
    ...
    $('#newsTable').DataTable({
      ...
      aaSorting : [[1, 'desc']],
      ajax: {
        url: this.server.baseUrl + 'manager/get_news',
        type: 'POST',
    ...
    table.on('click', '.remove', function (e: any) {
      swal({
        ...
        const $tr = $(this).closest('tr');
        const data = table.row($tr).data();
        thisObj.server.delete('manager/delete_news/' + data.id, true).subscribe(
          (resp) => {
