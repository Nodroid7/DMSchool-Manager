...
export class CanteenProductsComponent implements OnInit {
  ...
  deleteImg(thisImg){
    var thisObj = this;
    this.serverService.post('manager/delete_product_img/', {
  ...
  uploadNewImages() {  
    ...
    this.serverService.post('manager/upload_product_img/'+this.selectedProductId, formData, true).subscribe(
      (resp) => {
        swal({
          title: 'Submit succeed',
          text: 'Sumit succeessfully done.',
  ...
  getCategories() {
    this.server.get('manager/get_category_list', true).subscribe(resp => {
      if (resp['code'] === 200) {
  ...
  getClassTypes() {
    this.server.get('manager/get_class_list', true).subscribe(resp => {
      if (resp['code'] === 200) {
  ...
  getMealTypes() {
    this.server.get('manager/get_meal_list', true).subscribe(resp => {
      if (resp['code'] === 200) {
  ...
  submit() {
    ...
    this.serverService.post('manager/' + this.request_type + '_product', {
      id: this.id,
  ...
  submitOption() {
    this.serverService.post('manager/update_product_option', {
      id: this.id,
  ...
  ngAfterViewInit(): void {
    ...
    $('#productsTable').DataTable({
      ...
      aaSorting : [[1, 'desc']],
      ajax: {
        url: this.server.baseUrl + 'manager/get_products',
        type: 'POST',
    ...
    table.on('click', '.remove', function (e: any) {
      swal({
        ...
        const $tr = $(this).closest('tr');
        const data = table.row($tr).data();
        thisObj.server.delete('manager/delete_product/' + data.id, true).subscribe(
          (resp) => {
