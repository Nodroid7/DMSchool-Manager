...
export class CanteenCategoriesComponent implements OnInit {
  ...
  submit() {
    this.serverService.post('manager/' + this.request_type + '_category', {
      id: this.id,
      name: this.name,
      status: this.status
    }, true).subscribe(
  ...
  ngAfterViewInit(): void {
    ...
    $('#categoriesTable').DataTable({
      ...
      aaSorting : [[1, 'desc']],
      ajax: {
        url: this.server.baseUrl + 'manager/get_categories',
        type: 'POST',
    ...
    table.on('click', '.remove', function (e: any) {
      swal({
        ...
        const $tr = $(this).closest('tr');
        const data = table.row($tr).data();
        thisObj.server.delete('manager/delete_category/' + data.id, true).subscribe(
          (resp) => {
