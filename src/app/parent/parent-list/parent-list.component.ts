...
export class ParentListComponent implements OnInit {
  ...
  submit() {
    this.serverService.post('manager/' + this.request_type + '_parent', {
      id: this.id,
  ...
  ngAfterViewInit(): void {
    ...
    $('#parentsTable').DataTable({
      ...
      aaSorting : [[1, 'desc']],
      ajax: {
        url: this.server.baseUrl + 'manager/get_parents',
        type: 'POST',
    ...
    table.on('click', '.remove', function (e: any) {
      swal({
        ...
        const $tr = $(this).closest('tr');
        const data = table.row($tr).data();
        thisObj.server.post('manager/delete_parent', {id : data.id}, true).subscribe(
          (resp) => {
