...
export class ParentInfoComponent implements OnInit {
  ...
  getParentById(parentId) {
    this.server.post('manager/get_parent', {parentId: parentId}, true).subscribe(resp => {
      if (resp['code'] === 200) {
  ...
  getParentOrdersCountById(parentId) {
    this.server.post('manager/get_orders_count', {parentId: parentId}, true).subscribe(resp => {
      if (resp['code'] === 200) {
  ...
  getParentStudentsCountById(parentId) {
    this.server.post('manager/get_students_count', {parentId: parentId}, true).subscribe(resp => {
      if (resp['code'] === 200) {
  ...
  ngAfterViewInit(): void {
    ...
    $('#studentsTable').DataTable({
      ...
      aaSorting : [[1, 'desc']],
      ajax: {
        url: this.server.baseUrl + 'manager/get_students/'+this.parentId,
        type: 'POST',
    ...
    $('#ordersTable').DataTable({
      ...
      aaSorting : [[1, 'desc']],
      ajax: {
        url: this.server.baseUrl + 'manager/get_orders/'+this.parentId,
        type: 'POST',
        headers: {'Authorization': this.authService.getToken()},
        dataSrc: 'data',
        error: function(jqXhr, textStatus, errorThrown) {
    ...
    ordersTable.on('click', '.more', function (e: any) {
      ...
      thisObj.server.post('manager/get_order_detail', {orderId: data.id}, true).subscribe(resp => {
        if (resp['code'] === 200) {
